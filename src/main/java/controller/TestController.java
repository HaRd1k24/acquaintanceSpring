...
public class TestController {
    ...
    /*
     curl -X 'GET' \http://localhost:8080/post?v=2;
    ...
    public HelloResponse Version(@PathVariable String name, @RequestParam(value = "v", required = false) Integer version) {
        ...
        if (version == null) {
            ...
            switch (version) {
                case 0:
                    response.setMessage("version 0");
                    break;
                case 1:
                    response.setMessage(name + " Hello version : 1");
                    break;
                case 2:
                    response.setMessage(" Hi, nice to meet you " + name + ", you version application 2");
                    break;
                default:
